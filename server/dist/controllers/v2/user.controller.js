...
export const registerUser = async (req, res) => {
    ...
    if ([username, email, fullname, password].some((str) => !str || str.trim().length === 0) ||
    ...
    if (password.length < Number(process.env.PASSWORD_MIN_LENGTH || "6") ||
        password.length > Number(process.env.PASSWORD_MAX_LENGTH || "16")) {
        return res
            .status(400)
            .json(new ApiResponse(400, `Password must be between ${process.env.PASSWORD_MIN_LENGTH} and ${process.env.PASSWORD_MAX_LENGTH} characters`, null, false));
    }
    try {
...
export const createPost = async (req, res) => {
    ...
    try {
        ...
        if (!createdPost) {
            ...
            return res
                .status(500)
                .json(new ApiResponse(500, "Failed to create post", null, false));
        }
        res.status(200).json(new ApiResponse(200, "Post created successfully", createdPost, true));
    }
    catch (error) {
        console.log(error);
        return res
 ...
 */
export const showPostsByPageNumber = async (req, res) => {
    const user = req.user;
    ...
    try {
        ...
        return res
            .status(200)
            .json(new ApiResponse(200, "Posts retrieved successfully", posts, true));
    }
    catch (error) {
        console.log(error);
        return res
    ...
    try {
        ...
        const fetchProfile = await prisma.user.findUnique({
            ...
            select: {
                ...
                posts: req.query.includePosts === "true"
                    ? {
                        select: {
                            id: true,
                            title: true,
        ...
        if (!fetchProfile) {
            return res
                .status(404)
                .json(new ApiResponse(404, "User profile not found", null, false));
        }
        ...
        res.status(200).json(new ApiResponse(200, "User profile retrieved", profile, true));
    }
    catch (error) {
        console.log(error);
        return res
    ...
    try {
        ...
        const profile = {
            user: {
                ...
                isFollowing: Array.isArray(fetchProfile.followers) &&
 ...
 */
export const profilePagePosts = async (req, res) => {
    const user = req.user;
    const pageNumber = parseInt(req.params.page || "1");
    const postsPerPage = parseInt(process.env.POST_PER_PAGE || "5");
...
export const searchUsers = async (req, res) => {
    ...
    if (!query || typeof query !== "string" || query.trim().length === 0) {
        return res
            .status(400)
            .json(new ApiResponse(400, "Query parameter is required", null, false));
    }
    try {
        ...
        res.status(200).json(new ApiResponse(200, "Users retrieved successfully", users, true));
    }
    catch (error) {
        console.log(error);
        return res
